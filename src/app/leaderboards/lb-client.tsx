...
const EntryWrapped = ({ lbEntry, position }: { lbEntry: LeaderboardEntry, position: number }) => {
  ...
  return (
    <tr className="hover:scale-105 cursor-pointer ease-in transition-transform" key={position}>
      <td className={`py-2 ${bgStyle} ${borderStyle} rounded-l border-y border-l`}>{position + 1}.</td>
      <td className={`border-y ${borderStyle} ${bgStyle}`}>{lbEntry.name}</td>
...
export function LbClient() {
  ...
  const entries: LeaderboardEntry[] = [{name: 'Negus', score: 69}, {name: 'Negus', score: 69}, {name: 'Negus', score: 69}, {name: 'Negus', score: 69}];

  return (
    <main className={`h-screen ${luckiestGuy} flex justify-center items-center w-screen`}>
      <div className='w-[50vw] h-[60vh] gap-y-4 flex flex-col rounded border-[#7F5B3B] border-2 from-[#BF945A] to-[#B48145] bg-gradient-to-b'>
