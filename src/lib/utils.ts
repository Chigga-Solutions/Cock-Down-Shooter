...
export function decideBetweenDir<T>(a: T, b: T, c: T, d: T): T {
  return Math.random() > 0.75 ? a : Math.random() > 0.5 ? b : Math.random() > 0.25 ? c : d;
}

export type Direction = 'up' | 'down' | 'left' | 'right';
export type Coord = [number, number]; // x, y (%)

export function generateChickenCoords(width: number, height: number): [Coord, Coord, Direction, Direction] {

  let startSide: Direction = decideBetweenDir('up', 'down', 'left', 'right');
  let endSide: Direction = decideBetweenDir('up', 'down', 'left', 'right');
  if (startSide === endSide) {endSide = decideBetweenDir('up', 'down', 'left', 'right');}
  ...
  switch(endSide) {
    ...
    case 'down': {
      ...
      else if (startSide === 'left') coordXe = randomBetween(0, 100 - coordYs);
      else coordXe = randomBetween(0, 100);
    } break;
    case 'left': {
      coordXe = 0;
